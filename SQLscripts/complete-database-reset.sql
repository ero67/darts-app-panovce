...
CREATE POLICY "Users can view groups of their tournaments" ON groups
    FOR SELECT USING (
        EXISTS (
...
-- Create RLS policies for dart_throws
CREATE POLICY "Users can view dart throws of their tournament legs" ON dart_throws
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM legs 
            JOIN matches ON matches.id = legs.match_id
            JOIN groups ON groups.id = matches.group_id
            JOIN tournaments ON tournaments.id = groups.tournament_id
            WHERE legs.id = dart_throws.leg_id 
            AND tournaments.user_id = auth.uid()
        )
    );

CREATE POLICY "Users can create dart throws for their tournament legs" ON dart_throws
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM legs 
            JOIN matches ON matches.id = legs.match_id
...
CREATE POLICY "Users can view tournament players of their tournaments" ON tournament_players
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM tournaments 
...
-- Create RLS policies for group_players
CREATE POLICY "Users can view group players of their tournaments" ON group_players
    FOR SELECT USING (
        EXISTS (
...
-- Create RLS policies for match_player_stats
CREATE POLICY "Users can view match player stats of their tournaments" ON match_player_stats
    FOR SELECT USING (
        EXISTS (
...
CREATE POLICY "Users can create match player stats for their tournaments" ON match_player_stats
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM matches 
            JOIN groups ON groups.id = matches.group_id
...
CREATE POLICY "Users can update match player stats of their tournaments" ON match_player_stats
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM matches 
            JOIN groups ON groups.id = matches.group_id
...
-- Create RLS policies for group_standings
CREATE POLICY "Users can view group standings of their tournaments" ON group_standings
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM groups 
            JOIN tournaments ON tournaments.id = groups.tournament_id
...
CREATE POLICY "Users can view tournament stats of their tournaments" ON tournament_stats
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM tournaments 
